  ...
  $.fn.anim = function(properties, duration, ease, callback, delay){
    ...
    if (typeof properties == 'string') {
      ...
      if (duration > 0 && typeof properties === 'object') {
        ...
        cssValues[transitionTiming2] = (ease || 'linear')
      }
    }
...
$(function(){
  var nav_map = {
    "js_about_us_nav" : [1,2,3,4],
    "js_produce_nav" : [5,6,7],
    "js_trend_nav" : [8,9],
    "js_coopertion_nav" : [10,11]
  }
  onePageScroll("#js_main",{
    loop: true,
    pagination: false,
    afterMove: function(index) {
      for(var key in nav_map){
        if($.inArray(index,nav_map[key]) != -1){
          $("#js_catalog_list a").removeClass("active");
          $("#"+key).addClass("active");
        }
      }
    }
  });
  $("#js_close_catalog").click(function(){
    $("#js_main_catalog").hide();
  });
  $(".js_show_catalog").click(function(){ 
    $("#js_main_catalog").show();
  });
  $(".js_article").click(function(){
    $("#js_main_trend").show();
    var id=$(this).attr("ref_id");
    $.post("wap/get_artcile?id="+$(this).attr("ref_id"),{},function(json){
      if(json.status == "success"){
        $("#js_main_trend").attr("ref_id",id);
        $("#js_trend_img").attr("src",json.img_url);
        $("#js_trend_text").html(json.text);
      }else{
        alert("没有文章了");
      }
    },"json");
  });
  $("#js_trend_close").click(function(){
    $("#js_main_trend").hide();
  });
  $(".js_turn").click(function(){
    var id = parseInt($("#js_main_trend").attr("ref_id"));
    ...
    $("#js_main_trend").attr("ref_id",id);
    $.post('wap/get_artcile/?id='+id,{},function(json){
      if(json.status == "success"){
        $("#js_trend_img").attr("src",json.img_url);
        $("#js_trend_text").html(json.text);
      }else{
  ...
  $("#js_about_us_nav").click(function(){
    $("#js_main").moveTo(1);
    $("#js_main_catalog").hide();
  });
  $("#js_produce_nav").click(function(){
    $("#js_main").moveTo(5);
    $("#js_main_catalog").hide();
  });
  $("#js_trend_nav").click(function(){
    $("#js_main").moveTo(8);
    $("#js_main_catalog").hide();
  });
  $("#js_coopertion_nav").click(function(){
    $("#js_main").moveTo(10);
    $("#js_main_catalog").hide();
  });
});